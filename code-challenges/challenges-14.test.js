...
const screenForNames = (arr) => {
  ...
  return arr.filter(elem => {
    return /^[DM][rs]s?\.\s[^\sD]\D*/.test(elem);
  });
}
...
let biggerThanLuke = (arr) => {
  ...
  let lukeMass = 0;
  for (let elem of arr) {
    if (elem.name.includes('Luke')) {
      lukeMass = parseInt(elem.mass);
    }
  }
  return arr.filter(elem => {
    return parseInt(elem.mass) > lukeMass;
  }).map(elem2 => elem2.name).join(' - ');
};
...
const detectTicTacToeWin = (board) => {
  ...
  for (let row of board) {
    if (row.every(elem => elem == 'X') || row.every(elem => elem == 'O')) {
      return true;
    }
  }
  let colms = board.map((elem, idx) => {
    return [board[0][idx],board[1][idx],board[2][idx]]
  });
  for (let col of colms) {
    if (col.every(elem => elem == 'X') || col.every(elem => elem == 'O')) {
      return true;
    }
  }
  let diagnoals = [
  ...
  for (let diag of diagnoals) {
    if (diag.every(elem => elem=='X') || diag.every(elem=>elem=='O')) {
      return true;
    }
  }
  return false;
